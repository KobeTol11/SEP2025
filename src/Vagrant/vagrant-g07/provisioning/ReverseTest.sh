...
http {
    ...
    resolver 192.168.207.54 valid=300s;  # External resolver for OCSP stapling
...
server {
    ...
    location / {
        proxy_pass http://192.168.207.55;
        proxy_redirect off;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_set_header Host \$http_host;
...
server {
    ...
    location / {
        proxy_pass http://192.168.207.61;
        proxy_redirect off;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_set_header Host \$http_host;
...
if ! grep -q "192.168.207.60 www.g07-syndus.internal" /etc/hosts; then
    echo "192.168.207.60 www.g07-syndus.internal" | sudo tee -a /etc/hosts
    echo "Added g07-syndus.internal to /etc/hosts"
fi

GATEWAY_IP="192.168.207.17"
INTERNAL_NETWORK="192.168.207.0/28"
